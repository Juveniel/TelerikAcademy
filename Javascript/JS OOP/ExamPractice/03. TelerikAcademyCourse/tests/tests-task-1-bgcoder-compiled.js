...
describe('Test for Modules and Patterns in JavaScript', function () {
	...
	describe('Title validation checks', function () {
		...
		it('expect not to throw if titles are legit and contain all kinds of symbols', function () {
			function test() {
				var jsoop = Object.create(result);
				jsoop.init('!@#$ %^&*(', [getValidTitle() + ')(*&^%$#%^&', getValidTitle() + 'lfeopwkfie hfy3r7 38r 4j', getValidTitle() + 'lfeop,,wkfie hfy3r7 38r 4j', getValidTitle() + '-----====-----']);
			}
			expect(test).not.to.throw();
		});
	});
	...
	describe('Getting students', function () {
		...
		it('expect getAllStudents to return an array of the listed students (no students)', function () {
			var jsoop = Object.create(result).init(getValidTitle(), [getValidTitle()]);
...
var validTitles = ['Modules and Patterns', 'Ofresult, this is a valid title!', 'No errors hIr.', 'Moar taitles', 'Businessmen arrested for harassment of rockers', 'Miners handed cabbages to the delight of children', 'Dealer stole Moskvitch', 'Shepherds huddle', 'Retired Officers rally', 'Moulds detonate tunnel', 'sailors furious'],
...
//# sourceMappingURL=tests-task-1-bgcoder-compiled.js.map